...
The IBM MQ Simulator now supports dynamic template parameters in XML messages. This allows you to generate unique values for each message sent, perfect for load testing and realistic data simulation.
...
### Integration Points
- **MqService.sendMessage()**: Applies templates to single messages
- **MqService.sendMessagesMultiThreaded()**: Applies templates to batch messages
- **MainStage.validateXml()**: Detects and displays template usage

## Best Practices

1. **Use Consistent Ranges**: Keep min-max ranges realistic for your use case
2. **Combine Static and Dynamic**: Mix fixed values with templates for realistic data
3. **Test Template First**: Send 1 message to verify template output before batch sending
4. **Document Templates**: Add XML comments to explain what each placeholder represents
5. **Use Appropriate Types**: Choose NUMBER for IDs, AMOUNT for prices, etc.

## Limitations

- Placeholders are case-insensitive (${ID} = ${id})
- Cannot nest placeholders (${NUMBER:${MIN}-${MAX}} not supported)
- Date/time values use system timezone
- Generated names are from a predefined list (24 first names, 24 last names)
